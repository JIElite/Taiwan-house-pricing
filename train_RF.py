...
if MLFLOW:
    ...
    mlflow.start_run(run_name='RF Regressor')
    mlflow.log_params({'model_type': RandomForestRegressor.__name__,
...
model = RandomForestRegressor(n_estimators=100, criterion='squared_error',
                              max_depth=MAX_DEPTH, bootstrap=True, oob_score=True)
model.fit(X_train, y_train)

print('Training performance: ')
r2, mae, mse = simple_evaluate(
    model, X_train[-30000:], y_train[-30000:], verbose=True)
if MLFLOW:
    mlflow.log_metrics(
