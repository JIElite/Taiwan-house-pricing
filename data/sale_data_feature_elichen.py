...
remaining_cols = list(columns_map.values()) + ['編號', '建物型態', '電梯']
...
def preprocess_features(path, out_path):
    ...
    df['Elevator'] = df[['建物型態', '電梯']].apply(encode_elevator, axis=1)
    ...
    df.drop(columns=['Main_Usage', 'Building_Materials', '建物型態', '電梯'], inplace=True)
    df.to_csv(out_path)
    return df

sale_data = preprocess_features('./sale_data.csv', out_path='./data/sale_data_feature_elichen.csv')
